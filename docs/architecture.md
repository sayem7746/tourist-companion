...
- **Frontend**: trip onboarding, dashboard, arrival checklist, concierge chat, nearby, itinerary, safety, ops login (`/admin/login`, `/admin`), and content CMS (`/admin/content`).
- **Backend**: REST API with auth (tourist and admin JWT roles; `ADMIN_TOKEN` still accepted on admin routes), trip context, places, itinerary, public partner listings (`GET /partners`, `GET /partners/:id`), referrals (`POST /referrals/clicks`, `POST /referrals/leads`, `GET /r/:code`), partner admin (`/admin/partners`), referral analytics (`GET /admin/referrals/analytics`), content admin (`/admin/content`) for arrival guides, FAQs, etiquette, payment guidance, and safety copy, a public emergency directory (`GET /emergency`), a searchable embassy/consulate directory (`GET /embassies`), and a tourist safety guide (`GET /safety`).
- **Database**: PostgreSQL (`users` with `role` `tourist|admin`, `tourist_profiles`, `trips`, `places`, `trip_places`, `itineraries`, `itinerary_days`, `itinerary_items`, `providers` with marketplace listing columns, `referrals` with channel tracking, `arrival_checklist_items`, `content_items`, `concierge_messages`). Migrations live in `backend/migrations/`.
