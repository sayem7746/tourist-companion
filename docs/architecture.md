...
- **Backend**: REST API with auth, trip context, places, itinerary, public partner listings (`GET /partners`, `GET /partners/:id`), referrals (`POST /referrals/clicks`, `POST /referrals/leads`, `GET /r/:code`), partner admin (`/admin/partners`), and referral analytics (`GET /admin/referrals/analytics`).
